...
class FinanceScreen extends StatelessWidget {
  ...
  String _fmt(double v, NumberFormat f) =>
      v >= 1000000000 ? '${(v / 1000000000).toStringAsFixed(1)}B\nVNĐ' : '${f.format(v)}\nVNĐ';
        ...
        child: CustomScrollView(slivers: [
          ...
          SliverPadding(
            ...
            sliver: SliverList(delegate: SliverChildListDelegate([
              ...
              if (p.paymentReceipts.isEmpty && p.feeDetails.isEmpty && p.feeSummaries.isEmpty)
                SurfaceCard(child: Center(child: Padding(
                  padding: const EdgeInsets.all(24),
                  child: Column(children: [
                    Text('Chưa có dữ liệu học phí',
                        style: TextStyle(color: AppTheme.onSurfaceVariant)),
                    const SizedBox(height: 8),
                    Text('Receipts: ${p.paymentReceipts.length}, Details: ${p.feeDetails.length}, Summaries: ${p.feeSummaries.length}',
                  ...
                  child: SurfaceCard(
                    ...
                    child: Row(children: [
                      ...
                      Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                        Text(_fmt(soTien, f),
                            style: Theme.of(context).textTheme.titleSmall
                                ?.copyWith(fontWeight: FontWeight.w700, color: const Color(0xFF2E7D32), height: 1.2),
              ...
              if (p.feeDetails.isNotEmpty) ...[
                ...
                ...(() {
                  ...
                  print('📋 After filter: grouped=${grouped.length}, filtered=$filtered');
                  return grouped.entries.map((entry) => Column(
                    ...
                    children: [
                      ...
                      ...entry.value.map((d) {
                        final tenHocPhan = d['ten_hoc_phan']?.toString() ?? 'Chưa xác định';
                        final phaiNop    = (d['so_tien_phai_nop'] as num?)?.toDouble() ?? 0;
                        final daNop      = (d['so_tien_da_nop'] as num?)?.toDouble() ?? 0;
                        final chenhlech  = daNop - phaiNop;
                        final trangThai  = d['trang_thai']?.toString() ?? '';
                        final daDong     = chenhlech >= 0;
                        final trangThaiText = daDong ? 'Đã thanh toán' : 'Chưa thanh toán';
                            ...
                            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                              Row(children: [
                                Container(
                                  ...
                                  child: const Icon(Icons.book_outlined, size: 18, color: AppTheme.primary),
                                ),
                                ...
                                Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                                  Text('${f.format(chenhlech.abs())}\nVNĐ',
                                      style: Theme.of(context).textTheme.labelMedium
                                          ?.copyWith(fontWeight: FontWeight.w700, 
                                              color: daDong ? const Color(0xFF2E7D32) : AppTheme.tertiary,
                                              height: 1.2),
                                      textAlign: TextAlign.right,
                                      maxLines: 2),
                                ]),
                              ...
                              Row(children: [
                                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                  Text('Phải nộp: ${f.format(phaiNop)} VNĐ',
                                      style: TextStyle(fontSize: 11, color: AppTheme.onSurfaceVariant, height: 1.2),
                                      maxLines: 2),
                                  const SizedBox(height: 2),
                                  Text('Đã nộp: ${f.format(daNop)} VNĐ',
                                ...
                                Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                                  Container(
                                    ...
                                    decoration: BoxDecoration(
                                      color: daDong ? const Color(0xFF2E7D32).withOpacity(0.12) : AppTheme.surfaceContainerLow,
                                      borderRadius: BorderRadius.circular(6),
                                    ),
                                    child: Text(trangThaiText,
                                        style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700,
                                            color: daDong ? const Color(0xFF2E7D32) : AppTheme.onSurfaceVariant,
                                            height: 1.2),
                                        textAlign: TextAlign.center),
                                  ),
                                ]),
